...
-- QUESTION 1  Join the necessary tables to find the total quantity of 
...
-- QUESTION 5 : Determine the top 3 most ordered pizza types based on revenue.
...
-- OUESTION 1 : Calculate the percentage contribution of each pizza type to total revenue.
...
select orders.date,
sum(revanue) over(order by orders.date) as cum_revanue
from
(select orders.date,
...
group by orders.date,pizzas.price) as sales ;



-- QUESTION 3 : DETERMINE THE TOP 3 MOST ORDERED PIZZA TYPE BASED ON REVANUE FOR EACH 
--               PIZZA CATEGORY.
